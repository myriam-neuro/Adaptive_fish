...
def load_and_sequence(filenames, data_points, sequence_size, filetype, number_of_fish=1, fish_position=[5, 6]):
    '''
    Load and sequences files of single fish trajectories

    :param filenames: list of the name of the files of the fish trajectories
    :param data_points: number of datapoints to take from the files
    (largest number of datapoints in the file than can be divided by the sequence_size)
    ...
    :param fish_position: in which colums is the actual fish data (default is columns [5,6]
    :return: array of size (number_of_sequence, sequence_size, 2), which the data cut into sequences fit to go into RNN model
    '''
    # create the arrays
    fish_complete = np.zeros((len(filenames) * number_of_fish, data_points, 2))
    number_of_sequences = int(data_points / sequence_size * number_of_fish * len(filenames))
    Training_data = np.empty((number_of_sequences, sequence_size, 2))
...
def load_and_sequence_partners(filenames, data_points, sequence_size, filetype,
    ...
    '''
    Load and sequences files  of fish and its partner trajectories
    :param filenames: list of the name of the files of the fish trajectories
    :param data_points: number of datapoints to take from the files
    (largest number of datapoints in the file than can be divided by the sequence_size)
    :param sequence_size: size of the sequence to cut the trajectories into
    :param filetype: type of the file (csv or xlsx)
    :param filenames_partner : if the data is not in the samefile as the focal fish (for the simulated data, it should go with filetype = csv)
    :param continuous_recording  : if the data is continuous in its tracking or is there NaN values
    :return: sequenced data of the two fishes in the order focal fish - partner fish (real fish) shape (number of sequences, sequence_size, 4)
    '''
    # create the arrays
...
def sequence(fish_1,fish_2,data_points,sequence_size,continuous_recording=True):
    ...
    :param fish_1: focal fish trajectory
    :param fish_2: real fish (partner) fish trajectory
    :param data_points: len(fish trajectories)
    :param sequence_size: length of the sequences necessary
    :param continuous_recording: if the data doesn't have NaN values, default is True
    :return: array of the sequences of the fish trajectories, shape (number of sequences, sequence_size, 4)
    first 2 coordinates focal fish and next 2 real partner fish coordinates
    """
...
def find_continuous_segments(data, min_length):
    """
    In a data set with NaN, finds continuous sequences of a defined length (min_length) and return the indices of the start of thoses sequences
    :param data: dataset to be processed
    :param min_length: length of the sequence required
    :return: array of the start indices of every continuous sequence in the dataset
    """
...
def transform_shuffle(data,labels, partner):
    ...
    :param data: dataset to be processed
    :param partner: True or False, if True it assumes the partner data are the last two coordinates of the dataset
    :return: shuffled and randomly rotated data
    """
